...
- **Bekende openstaande productiepunten** (uit `DEPLOY.md`): PWA-icons ✅ gegenereerd én live (`public/icon-192.png`, `public/icon-512.png`, `src/app/apple-icon.png` uit `scripts/icon-source.svg`); preview-env-vars nog niet geïmporteerd; handmatige smoke tests (admin-login, foto-upload, push) nog te doen.
...
### FB12 · Homepage-blokjes + toernooi-infopagina ✅ AF (deploy openstaand)
**Beslissing Merel (2026-07-03):** losse **nieuwe pagina `/toernooi`**.
- [x] **Nieuwe pagina `src/app/[locale]/toernooi/page.tsx`:** `force-dynamic`, `assertPageVisible("toernooi")`, volledig bewerkbaar via `/admin/teksten` (namespace `Toernooi`: title/lead + 3 secties introHeading/Body, formatHeading/Body, dayHeading/Body). Secties renderen alleen als de body gevuld is (`hasText`-patroon, zoals FB7), zodat een leeggemaakt veld geen leeg blok geeft. Zinvolle default-copy meegeleverd (NL+EN).
- [x] **Registry `src/lib/pages.ts`:** `toernooi` toegevoegd aan `PageKey` + `TOGGLABLE_PAGES` (adminLabel "Toernooi-info") → verschijnt automatisch in `/admin/zichtbaarheid` en filtert overal mee. Standaard zichtbaar (ontbrekende rij = zichtbaar).
- [x] **Nav-item** in `[locale]/layout.tsx` (na Aanmelden), **`Nav.toernooi`** + **`PageMeta.toernooi`** (SEO) in beide `messages/*.json`, en **`toernooi`** toegevoegd aan `sitemap.ts` (`STATIC_PATHS`).
- [x] **Homepage-blokjes:** nieuwe prominente sectie in `[locale]/page.tsx` met 3 ink-blokken (**Aanmelden / Over het toernooi / Regels**), gefilterd op `PageVisibility` (net als de feature-cards). Labels/omschrijvingen bewerkbaar via `Home.primary.*` in beide `messages/*.json`.
- [x] **Copy-check:** toernooi-copy noemt bewust géén hard teamaantal (de hero zegt "8 teams van 8 spelers" = rostergrootte; de regelspagina "5 skaters op de baan" = on-track) om tegenspraak te vermijden. En/em-dash vermeden (Merel's voorkeur).
- [x] **Echte event-info ingevuld (Merel leverde flyers van vorig jaar, event = identiek).** Toernooi-namespace herschreven met 5 secties: intro, **Opzet** (8 teams van 8 skaters, 2 poules, 3 poulewedstrijden/team, 13 wedstrijden incl. finale), **De wedstrijden** (20 min zonder klokstop, 1 team time-out, geen official review, foul out na 4 penalty's, wit-zwart in poule / eigen kleur in finale), **Programma** (voorlopig: 11:00 officials meeting, 12:00 start, 18:00 eind; poules wisselen af → min. 20 min tussen wedstrijden + 1× half uur pauze) en **Officials** (Head Ref: Nat Safe For Work, Head NSO: TBA). Datum van vorig jaar (29-06-2024) bewust NIET overgenomen. Alles bewerkbaar via `/admin/teksten`.
- [x] **Venue-pagina:** InfoCard "Vloer & wielen" toegevoegd (sportvloer met skatecoating, wielhardheid 92a tot 95a) uit dezelfde flyer. `Venue.floorTitle/floorBody` in beide `messages/*.json`.
- Lokaal end-to-end geverifieerd (screenshot home + /toernooi NL+EN met echte info, venue-card, admin lijst + tekst-namespace), `npm run build` groen (route `/[locale]/toernooi` aanwezig). Eerste versie live gedeployed (`8b9de2b`); content-update erna.
- **NB dev-flakiness (niet in prod):** tijdens deze sessie gaf de Next 16 dev-server bij snelle/gelijktijdige requests intermitterende 500's ("Unexpected non-whitespace character after JSON" — een torn read van een interne Next routes-manifest, o.a. verergerd door parallel `npm run build` naast `next dev` in dezelfde `.next`). Fix: dev killen, `rm -rf .next`, opnieuw starten (niet tegelijk builden). Geen appcode-bug, geen prod-impact.
...
- PWA-icons ✅ gegenereerd uit het logo en live gedeployed (zie §1).
