  ...
  return (
    <>
      <Card title="Eerst uitzoeken" sub="Punten waar het draaiboek zichzelf tegenspreekt of waar iets ontbreekt. Vink af zodra het besloten is.">
        {issues.map((i) => (
          <label key={i.id} className={`flex items-start gap-2 py-1 text-sm ${i.done ? "line-through text-derby-ink/50" : ""}`}>
            <input type="checkbox" className="mt-1 accent-derby-accent" checked={i.done} onChange={(e) => patch(i.id, { done: e.target.checked })} />
            {i.text}
          </label>
        ))}
      </Card>
